...
def run_vqe():
    ...
    qnode = qml.QNode(qgcnn, qgcnn_dev)

    iterations = 0
    optimizer = qml.AdamOptimizer(stepsize=0.5)
    steps = 10
    qgrnn_params = list([np.random.randint(-20, 20) / 50 for i in range(0, 10)]) # @Todo: possibly change parameter shape
...
def qgcnn(params1, params2, time=None):
    ...
    for i in range(0, int(depth)):
        qgcnn_layer(params1, params2, reg2, new_ising_graph, trotter_step)
...
def qgcnn_layer(param1, param2, qubits, graph, trotter_step):
